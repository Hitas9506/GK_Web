...
const CATEGORY_LABELS: Record<string, string> = {
  "dien-thoai": "Điện Thoại", tablet: "Máy Tính Bảng", "tai-nghe": "Tai Nghe", "phu-kien": "Phụ Kiện",
};
...
export default function OrdersPage() {
  ...
  const cancelOrder = (id: string) => {
    ...
    localStorage.setItem("technext_orders", JSON.stringify(updated));
    setOrders((prev) => prev.map((o) => o.id === id ? { ...o, status: "cancelled" as OrderStatus } : o));
  };

  const clearOrders = () => {
    if (user.role === "admin") {
      localStorage.removeItem("technext_orders");
      setOrders([]);
    } else {
      const raw: Order[] = JSON.parse(localStorage.getItem("technext_orders") ?? "[]");
      const kept = raw.filter((o) => o.userId !== user.id);
      localStorage.setItem("technext_orders", JSON.stringify(kept));
      setOrders([]);
    }
  };
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "0.65rem", marginBottom: "1rem" }}>
                          {order.items.map((item, i) => (
                            <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                              <div style={{ width: "42px", height: "50px", background: "var(--color-muted)", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.5rem", flexShrink: 0 }}>
                                {item.product.category === "tai-nghe" ? "🎧" : item.product.category === "tablet" ? "📲" : item.product.category === "phu-kien" ? "⌚" : "📱"}
                              </div>
                              <div style={{ flex: 1 }}>
                                ...
                                <p style={{ fontSize: "0.72rem", color: "var(--color-text-muted)", margin: "0.1rem 0 0" }}>
                                  {CATEGORY_LABELS[item.product.category] ?? item.product.category} · {item.variant} · {item.color} · x{item.quantity}
                                </p>
                              </div>
