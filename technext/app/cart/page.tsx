...
export default function CartPage() {
  ...
  const [shipForm, setShipForm] = useState<ShippingInfo>(() => {
    if (typeof window !== "undefined") {
      const raw = localStorage.getItem("technext_user");
      if (raw) {
  ...
  const handleConfirm = () => {
    ...
    const existing: Order[] = JSON.parse(localStorage.getItem("technext_orders") ?? "[]");
    localStorage.setItem("technext_orders", JSON.stringify([order, ...existing]));
  ...
  if (items.length === 0) {
    return (
      <div style={{ paddingTop: "92px", minHeight: "80vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "1.5rem", padding: "100px 1.5rem 3rem", textAlign: "center" }}>
            ...
            <div key={`${item.product.id}-${item.variant}-${item.color}`} style={{ background: "white", borderRadius: "16px", border: "1px solid var(--color-border)", padding: "1.25rem", display: "flex", gap: "1.25rem", alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                  ...
                  <button onClick={() => removeItem(item.product.id, item.variant, item.color)} style={{ border: "none", background: "transparent", cursor: "pointer", color: "#ff4757", fontSize: "0.8rem", fontWeight: 600 }}>🗑 Xóa</button>
                </div>
              </div>
        ...
        <div style={{ background: "white", borderRadius: "20px", border: "1px solid var(--color-border)", padding: "1.5rem", position: "sticky", top: "90px" }}>
          ...
          {totalPrice < 2000000 && (
            <p style={{ fontSize: "0.78rem", color: "#e67e22", background: "#fef9e7", padding: "0.5rem 0.75rem", borderRadius: "8px", marginBottom: "1rem" }}>
              🚚 Mua thêm <strong>{formatPrice(2000000 - totalPrice)}</strong> để miễn phí vận chuyển!
            </p>
          )}
              ...
              <>
                <div style={{ padding: "1.25rem 1.5rem", display: "flex", flexDirection: "column", gap: "0.7rem" }}>
                  ...
                  {PAYMENT_METHODS.map((method) => {
                    ...
                    return (
                      <label key={method.id} style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "0.85rem 1rem", borderRadius: "12px", border: sel ? "2px solid var(--color-primary)" : "2px solid #e5e7eb", background: sel ? "var(--color-primary-light)" : "white", cursor: "pointer", transition: "all 0.18s" }}>
                        <input type="radio" name="payment" value={method.id} checked={sel} onChange={() => setSelectedMethod(method.id)} style={{ accentColor: "var(--color-primary)", width: "17px", height: "17px" }} />
                        ...
                        {sel && <span style={{ color: "var(--color-primary)", fontWeight: 700 }}>✓</span>}
                      </label>
                    );
                  })}
                </div>
                <div style={{ display: "flex", gap: "0.75rem", padding: "1rem 1.5rem 1.5rem", borderTop: "1px solid var(--color-border)" }}>
                  ...
                  <button onClick={() => setStep(2)} style={{ flex: 2, padding: "0.72rem", borderRadius: "10px", border: "none", background: "var(--color-accent)", color: "white", cursor: "pointer", fontWeight: 700, fontSize: "0.9rem", fontFamily: "inherit" }}>
                    Tiếp theo: Giao hàng →
                  </button>
                </div>
              </>
            ...
            {step === 2 && (
              <>
                <div style={{ padding: "1.25rem 1.5rem" }}>
                  ...
                  <ShippingForm value={shipForm} onChange={setShipForm} />
                </div>
                <div style={{ display: "flex", gap: "0.75rem", padding: "1rem 1.5rem 1.5rem", borderTop: "1px solid var(--color-border)" }}>
                  ...
                  <button
                    ...
                    style={{ flex: 2, padding: "0.72rem", borderRadius: "10px", border: "none", background: "var(--color-accent)", color: "white", cursor: "pointer", fontWeight: 700, fontSize: "0.9rem", boxShadow: "0 4px 14px rgba(0,0,0,0.2)", fontFamily: "inherit" }}>
                    Xác Nhận Đặt Hàng ✓
                  </button>
                </div>
              </>
